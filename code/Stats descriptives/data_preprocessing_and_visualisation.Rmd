...
scores_df %>% ggplot(aes(Y_reactivity)) + geom_density()
```

```{r}
# Density functions according to the nucleotides.
scores_df %>% ggplot(aes(Y_reactivity, fill=X)) + 
...
scores_df %>% ggplot(aes(Y_reactivity, group = X, fill = X)) + 
  geom_density() + facet_wrap(~X) + 
  theme(legend.position ="right")+labs(title="reactivity", fill="nucleotide")
```
...
```{r}
# Density function without grouping.
scores_df %>% ggplot(aes(Y_score_mg_ph10)) + geom_density()
```
...
```{r}
# Density function without grouping.
scores_df %>% ggplot(aes(Y_score_ph10)) + geom_density()
```
...
```{r}
scores_df %>% ggplot(aes(Y_score_mg_50c)) + geom_density()
```
...
```{r}
scores_df %>% ggplot(aes(Y_score_50c)) + geom_density()
```
