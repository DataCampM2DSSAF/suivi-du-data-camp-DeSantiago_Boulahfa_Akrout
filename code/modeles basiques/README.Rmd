...
```{r}
model_RF_reactivity = update(object = model_RF_reactivity,
                             x = data[, 2:118],
                             y = data$reactivity)
```

```{r}
model_RF_deg_Mg_pH10 = update(object = model_RF_deg_Mg_pH10,
                              x = (rbind(data_train, data_val))[, 2:118],
                              y = (rbind(data_train, data_val))$deg_Mg_pH10)
```

```{r}
model_RF_deg_pH10 = update(object = model_RF_deg_pH10,
                           x = (rbind(data_train, data_val))[, 2:118],
                           y = (rbind(data_train, data_val))$deg_pH10)
```

```{r}
model_RF_deg_Mg_50C = update(object = model_RF_deg_Mg_50C,
                             x = (rbind(data_train, data_val))[, 2:118],
                             y = (rbind(data_train, data_val))$deg_Mg_50C)
```

```{r}
model_RF_deg_50C = update(object = model_RF_deg_50C,
                          x = (rbind(data_train, data_val))[, 2:118],
                          y = (rbind(data_train, data_val))$deg_50C)
```

```{r}
y_test_pred = y_test_pred %>%
  mutate(reactivity = predict(model_RF_reactivity, data_test, type="response")) %>%
  mutate(deg_Mg_pH10 = predict(model_RF_deg_Mg_pH10, data_test, type="response")) %>%
  mutate(deg_pH10 = predict(model_RF_deg_pH10, data_test, type="response")) %>%
  mutate(deg_Mg_50C = predict(model_RF_deg_Mg_50C, data_test, type="response")) %>%
  mutate(deg_50C = predict(model_RF_deg_50C, data_test, type="response")) %>%
  mutate_all(unlist)
```
